    ...
    for (const pool of dexPools) {
      ...
      for (const sym of [t0, t1]) {
        ...
        opportunities.push({
          ...
          poolToken0: t0,
          poolToken1: t1,
          token0TvlUsd: Math.round(token0TvlUsd),
          token1TvlUsd: Math.round(token1TvlUsd),
          token0Tvl: pool.token0Tvl || 0,
          token1Tvl: pool.token1Tvl || 0,
          token0Pct: totalSideTvl > 0 ? Math.round((token0TvlUsd / totalSideTvl) * 100) : 0,
          token1Pct: totalSideTvl > 0 ? Math.round((token1TvlUsd / totalSideTvl) * 100) : 0,
